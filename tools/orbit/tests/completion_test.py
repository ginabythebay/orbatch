...
def _run_main(
    ...
    # Click infers the program name from these two; dev/orbit runs
...
def test_bash_source_emits_a_completion_script(
    ...
    code = _run_main(monkeypatch, ["dev/orbit"], {"_ORBIT_COMPLETE": "bash_source"})
    out = capsys.readouterr().out
    assert code == 0
    assert "complete -o nosort -F _devorbit_completion dev/orbit" in out
...
def test_help_is_unchanged_without_a_completion_var(
    ...
    code = _run_main(monkeypatch, ["dev/orbit", "--help"])
    out = capsys.readouterr().out
...
def test_main_parses_its_argument_not_sys_argv(
    ...
    monkeypatch.setattr(sys, "argv", ["dev/orbit", "--help"])
...
def test_bash_source_also_registers_the_console_script_name(
    ...
    assert registrations == [
        "complete -o nosort -F _devorbit_completion dev/orbit",
        "complete -o nosort -F _devorbit_completion orbit",
    ]
