"""The run dashboard as a Textual screen, so any app can host it: `batch run`
through `DashboardApp`, and orbit over its issue views."""
...
class DashboardScreen(Screen[None]):
    ...
    DashboardScreen #rows { height: 1fr; padding: 1 1 0 1; }
    DashboardScreen #footing { dock: bottom; height: 2; }
    DashboardScreen #status { height: 1; padding: 0 1; }
    """
    BINDINGS: ClassVar[list[BindingType]] = [
        ...
        Binding("escape", "close_screen", "Quit", show=False),
    ]

    class RunChanged(Message):
        """The run thread started or ended; `live` on the screen says which."""
    ...
    @property
    def _host(self) -> App[None]:
        return cast("App[None]", self.app)

    def install_on(self, app: App[None], name: str) -> None:
        """Installed, so a pop suspends the screen instead of destroying it and
        the host can push it back by name with its state intact."""
    ...
    def _drive(self) -> None:
        """A daemon thread, not a Textual worker: quitting the host must leave an
        in-flight VM running rather than block the exit on `Orchestrator.run`."""
        threading.Thread(target=self._consume, name="batch-run", daemon=True).start()
        self.post_message(self.RunChanged())
    ...
    def _update(self, selector: str, content: RenderableType) -> None:
        """A poll can land after the widgets are gone; a torn-down screen has
        nothing to say and must not raise out of a timer callback."""
    ...
    def action_close_screen(self) -> None:
        """Close what the developer is in: the screen when a host pushed it over
        its own, the app when this is all there is."""
        if len(self._host.screen_stack) > 1:
            _ = self._host.pop_screen()
        else:
            self._host.exit()
